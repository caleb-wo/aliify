...
class AddCommand extends Command {
  ...
  @override
  final String description = 'Adds a new alias.\nUsage: aliify add <name> <command>';
...
class RemoveCommand extends Command {
  ...
  @override
  final String description = 'Removes an alias by its position number.\nUsage: aliify remove <position>';
  ...
  void run() {
    final rest = argResults?.rest ?? [];
    if (rest.isEmpty) {
      usageException('You must provide the position number of the alias to remove.');
    }

    final position = int.tryParse(rest[0]);
    if (position == null || position < 1 || position > repo.aliases.length) {
      usageException('Invalid position number. Please check "aliify list" and try again.');
    }
...
class UpdateCommand extends Command {
  ...
  @override
  final String description = 'Updates an existing alias.\nUsage: aliify update <position> <new_name> <new_command>';
  ...
  void run() {
    final rest = argResults?.rest ?? [];
    if (rest.length < 3) {
      usageException('You must provide a position, a new name, and a new command.');
    }
